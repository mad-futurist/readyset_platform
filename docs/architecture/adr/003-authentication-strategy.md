...
Registration does not authenticate. Password login requires that password identity's verification; verification of another provider never upgrades it. `(provider, provider_subject)` is authoritative. A Google-verified normalized email may attach Google to the one active matching user. If its matching password identity remains unverified, Google proof consumes every verify-email token and deletes the unsafe password identity and cascading credential before the Google session is created. A verified password identity is retained. Subject/email conflicts, unverified claims, disabled users, and uniqueness races fail closed. Legacy sessions without identity provenance are invalidated.
...
## Consequences
M1 works locally and provider identities can migrate. ReadySet temporarily owns sensitive password/session/recovery operations and must meet a higher security/operational bar. Email delivery is a provider-neutral port: development/test captures messages and may expose raw tokens only when explicitly enabled; staging/production require TLS-capable SMTP when password auth or invitations are enabled. Database state commits before synchronous delivery. Registration and invitations report delivery failure without implying rollback; resend/reissue are recovery paths. Reset/resend remain non-enumerating. Password reset revokes all sessions and outstanding reset tokens.
